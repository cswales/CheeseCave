...
// And yet, even though I thought all the world might be ruled by JSON, I find that the world is still ruled
// by CSV. Let's output a CSV file.
...
func process(iFilename string, oFilename string, highLimitT float32, lowLimitT float32) {
	...
	// output a leading line of information
	out_file.WriteString("sensor,time,epoch,temperature,celsius,humidity\n" )
...
//		}
		...
		// for each line, drop a CSV line
		outlines++

		fmt.Fprintf(out_file, "%s, %s, %d,%.2f,%.2f,%.2f\n",
	...
	fmt.Printf(" found and parsed %v lines; output %v lines\n", lineNum,outlines)
